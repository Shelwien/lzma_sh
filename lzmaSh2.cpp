...
struct lzma_decode {
  ...
  lzma_decode( FILE* _f, FILE* _g ) {
    ...
    uint sym=0, dist, pos, len;

    for( qword filepos=0; filepos<f_len; ) {
      ...
      if( BIT(c_IsMatch[state][posState])==0 ) { // decode a literal?
        ...
        uint f_rep = BIT(c_IsRep[state]);

        if( f_rep==0 ) state += kNumStates; else {

          if( BIT(c_IsRepG0[state])==0 ) {

            if( BIT(c_IsRep0Long[state][posState])==0 ) {
              sym = dic[rep0pos()]; symstore(sym);
            ...
            if( BIT(c_IsRepG1[state]) ) {
              dist = rep2;
              if( BIT(c_IsRepG2[state]) ) dist = rep3, rep3 = rep2;
              rep2 = rep1;
            }
            rep1 = rep0; rep0 = dist;
          }
